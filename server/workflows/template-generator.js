...
export async function generateBackgroundTemplate({
  ...
  try {
    // Step 1: Create template record with 'generating' status
    db.prepare(`
    ...
    console.log('[TemplateGenerator] Template record created:', templateId);
 ...
 */
export function getTemplateWithAssets(templateId, db, onlySelected = false) {
  const template = db.prepare(`
    SELECT * FROM background_templates WHERE id = ?
  `).get(templateId);
 ...
 */
export async function refreshTemplateAssetUrls(templateId, db) {
  const storage = getStorage();
